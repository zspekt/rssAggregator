...
const getNextUrlsToFetch = `-- name: GetNextUrlsToFetch :many
SELECT
    url
FROM
    feeds
...
func (q *Queries) GetNextUrlsToFetch(ctx context.Context, limit int32) ([]string, error) {
	rows, err := q.db.QueryContext(ctx, getNextUrlsToFetch, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
